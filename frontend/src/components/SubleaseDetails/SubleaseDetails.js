...
const SubleaseDetails = () => {
    ...
    const subleaseID = parseInt(id, 10); // Convert the string ID to an integer for later use
    ...
    // Find the sublease post that matches the ID
    // TODO: REPLACE THIS WITH A DATABASE SEARCH FUNCTION THAT SEARCHES FOR SUBLEASES BASED ON ID
    const sublease = subleasePosts.find(post => post.id === subleaseID);
    if (!sublease) {
        return <div>Sublease not found.</div>; // Handle case where sublease is not found
    }
